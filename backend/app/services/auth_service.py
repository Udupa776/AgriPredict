...
def register_farmer(data: RegisterRequest) -> TokenResponse:
    ...
    existing = supabase.table("farmers").select("uid").eq("phone", data.phone).execute()
    if existing.data:
    ...
    farmer_id = str(result.data[0]["uid"])
...
def login_farmer(data: LoginRequest) -> TokenResponse:
    ...
    return TokenResponse(
        access_token=create_token(str(farmer["uid"])),
