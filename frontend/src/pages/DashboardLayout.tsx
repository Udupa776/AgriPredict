    ...
    <div className="flex min-h-screen bg-slate-50 font-sans text-slate-900 overflow-hidden">
      ...
      <aside className="w-64 bg-white border-r border-slate-200 flex flex-col shadow-sm relative z-40 hidden md:flex">
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link
                key={link.path}
                to={link.path}
                className={`flex items-center gap-3 px-4 py-3 rounded-xl font-semibold transition-all group ${active
        ...
        <div className="p-4 border-t border-slate-100 space-y-1">
          {/* <Link
            to="/success-stories"
            className={`flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all ${location.pathname.startsWith('/success-stories') ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-500 hover:bg-slate-50'
              }`}
          >
            <Icons.Success />
          ...
          {/* <Link
            to="/market"
            className={`flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all ${location.pathname.startsWith('/market') ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-100' : 'text-emerald-600 bg-emerald-50 hover:bg-emerald-100'
              }`}
          >
            <Icons.Market />
                  ...
                  <nav className="space-y-1">
                    {navLinks.map((link) => (
                      <Link
                        ...
                        className={`flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-sm transition-all ${location.pathname.startsWith(link.path) ? 'bg-green-600 text-white shadow-lg shadow-green-100' : 'text-slate-600 hover:bg-slate-50'
                          }`}
                      >
                        {link.icon}
                        {link.name}
                    ...
                    <div className="pt-4 mt-4 border-t border-slate-100 space-y-1">
                      ...
                      <Link
                        to="/success-stories"
                        onClick={() => setMobileMenuOpen(false)}
                        className={`flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-sm transition-all ${location.pathname.startsWith('/success-stories') ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-500'
                          }`}
                      >
                        <Icons.Success />
                        {t('common.success_stories')}
                      ...
                      <Link
                        to="/market"
                        onClick={() => setMobileMenuOpen(false)}
                        className={`flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-sm transition-all ${location.pathname.startsWith('/market') ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-100' : 'text-emerald-600 bg-emerald-50'
                          }`}
                      >
                        <Icons.Market />
                        {t('common.greenshift_market')}
